...
proc arima data=Work.preProcessedData plots
	...
	identify var= avgprice crosscorr=(maxtemp 'weather dummy'n);
	estimate p = 3 q = 5
	input=(4 $ maxtemp 5 $ maxtemp 'weather dummy'n) method=ML outest=work.outest0003 
		outstat=work.outstat0003;
	forecast lead=365 back=365 alpha=0.05 id=date1 interval=day;
	run;
quit;

ods select all;

%accuracy(indsn=work.out0003, timeid=date1, series=avgprice, 
    numholdback=&nhold);
run;
